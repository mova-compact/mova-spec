...
### 1. Core specification (`docs/concepts/mova_core.md`) — new §3.2
...
### 2. Global layer and verbs (`docs/concepts/mova_global_and_verbs.md`) — new §4.5 and §4.6
...
### 3. Security layer (`docs/concepts/mova_security_layer.md`) — new §12
...
**For policy authors**: to use action_signature-based rules in `ds.instruction_profile_core_v1`, set `target.kind = "action"` and specify `verb_id` and/or `tool_id` in the rule target. See `docs/concepts/mova_security_layer.md §12.3`.
